...
func TestNewDTokenBucketValidation(t *testing.T) {
	cli := redis.NewClient(&redis.Options{Addr: "127.0.0.1:6379"})

	if _, err := NewDTokenBucket("", 1, 4, cli); err == nil {
		t.Fatalf("expected error when redisKey is empty")
	}
	if _, err := NewDTokenBucket("bucket", 0, 4, cli); err == nil {
		t.Fatalf("expected error when rate <= 0")
	}
	if _, err := NewDTokenBucket("bucket", 1, 0, cli); err == nil {
		t.Fatalf("expected error when cap <= 0")
	}
	if _, err := NewDTokenBucket("bucket", 1, 4, nil); err == nil {
...
func TestNewDTokenBucket(t *testing.T) {
	...
	dt, err := NewDTokenBucket("dt_bucket", 2, 4, cli)
	if err != nil {
		t.Fatal(err)
	}
	for i := 0; i < 20; i++ {
